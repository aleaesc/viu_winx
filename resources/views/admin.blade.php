...
<body class="h-screen overflow-hidden">
    ...
    <div id="submission-modal" class="overlay-container hidden-page">
        <div class="white-card w-full max-w-4xl max-h-[90vh] overflow-y-auto p-8 md:p-10 relative fade-in">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 border-b border-gray-100 pb-6">
                ...
                <div><p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Email:</p><p class="font-bold text-lg text-black" id="modal-email">--</p></div>
                <div><p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Name:</p><p class="font-bold text-lg text-black" id="modal-name">-</p></div>
            </div>
            <div class="mb-8 pb-6 border-b border-gray-100">
                <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Services Voted:</p>
                <p class="text-lg text-gray-700" id="modal-service">--</p>
            </div>
    ...
    <div id="app-container" class="flex h-full hidden-page">
        ...
        <!-- SIDEBAR -->
        <aside id="admin-sidebar" class="w-64 bg-white border-r border-gray-100 flex flex-col h-full shadow-sm z-20 relative">
            ...
            <div class="p-8 pb-8 flex flex-col items-center">
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-black font-bold tracking-widest text-sm mt-1 sidebar-logo-text">ADMIN</span>
                </div>
            </div>
    ...
    <script>
            ...
            try {
                ...
                if(rRes && rRes.ok){
                    ...
                    responses = local.map(r => ({
                        ...
                        services: Array.isArray(r.services) ? r.services : (Array.isArray(r.genres) ? r.genres : null),
                        service: r.service || (Array.isArray(r.genres)? r.genres[0] : 'general'),
                        name: r.name || r.user_name || null,
                        email: r.email || null,
        ...
        function toggleSidebar() {
            ...
            ensureSidebarIcons('admin-sidebar');
            setTimeout(() => { lucide.createIcons(); ensureSidebarIcons('admin-sidebar'); }, 10);
            setTimeout(() => ensureSidebarIcons('admin-sidebar'), 50);
            setTimeout(() => ensureSidebarIcons('admin-sidebar'), 100);
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(collapsed) {
                document.getElementById('admin-sidebar')?.classList.add('sidebar-collapsed');
            }
            ensureSidebarIcons('admin-sidebar');
            setTimeout(() => { lucide.createIcons(); ensureSidebarIcons('admin-sidebar'); }, 10);
            setTimeout(() => { lucide.createIcons(); ensureSidebarIcons('admin-sidebar'); }, 100);
            setTimeout(() => { lucide.createIcons(); ensureSidebarIcons('admin-sidebar'); }, 300);
            setTimeout(() => { lucide.createIcons(); ensureSidebarIcons('admin-sidebar'); }, 600);
            setTimeout(() => ensureSidebarIcons('admin-sidebar'), 1000);
        });
        ...
        function renderSurveyAnswersTable() {
            ...
            dataRows.forEach((r, index) => {
                ...
                const servicesArr = Array.isArray(r.services) ? r.services : (Array.isArray(r.genres) ? r.genres : null);
                const singleService = (r.service||'').toString();
                const servicesDisplay = (servicesArr && servicesArr.length) ? servicesArr.join(', ') : singleService;
                row.innerHTML = `
                    <td class="pl-8 font-medium">${index + 1}</td>
                    <td class="location-cell">${(r.country||r.location_country||'-')}</td>
                    <td class="service-cell">${servicesDisplay}</td>
                    <td>${r.email || '-'}</td>
        ...
        function openSubmissionModal(index) {
            ...
            console.log('Single service:', r.service);
            ...
            const singleService = (r.service||'').toString();
            const servicesDisplay = (servicesArr && servicesArr.length) ? servicesArr.join(', ') : singleService;
            console.log('Final services display:', servicesDisplay);
            document.getElementById('modal-service').innerText = servicesDisplay || '--';
            document.getElementById('modal-email').innerText = (r.email||'').trim() || '-';
            const nameEl = document.getElementById('modal-name');
            const nameValue = (r.name||r.user_name||'').toString().trim();
        ...
        async function exportAnswersCSV(){
            const header = ['#','User Location','Services','Email','Date'];
            const rows = [header];
        ...
        async function exportAnswersPDF(){
            ...
            win.document.write('<table><thead><tr><th>#</th><th>User Location</th><th>Services</th><th>Email</th><th>Date</th></tr></thead><tbody>');
            (responses||[]).forEach((r,i)=>{
                const servicesArr = Array.isArray(r.services) ? r.services : (Array.isArray(r.genres) ? r.genres : null);
                const singleService = (r.service||'').toString();
                const servicesDisplay = (servicesArr && servicesArr.length) ? servicesArr.join(', ') : singleService;
                win.document.write('<tr><td>'+ (i+1) +'</td><td>'+ (r.country||'') +'</td><td>'+ servicesDisplay +'</td><td>'+ (r.email||'') +'</td><td>'+ ((r.submitted_at||r.date||'').toString().split('T')[0]) +'</td></tr>');
            });
            win.document.write('</tbody></table>');
