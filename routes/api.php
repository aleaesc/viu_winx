...
Route::post('/chatbot/ask', function (Request $request) {
    ...
    $question = is_string($raw) ? trim(strtolower($raw)) : '';
    $intent = 'fallback';
    $answer = '';

    if ($question === '') {
        $intent = 'empty';
        $answer = "I'm here to help with Viu. Ask me anything about subscriptions, downloads, devices, or account settings!";
    } elseif (preg_match('/\b(hi|hello|hey|kamusta|kumusta|yo|sup)\b/i', $question)) {
        $intent = 'greeting';
        $answer = "Hi, Viu Fam! 👋 I'm here to help! Ask me about subscriptions, downloads, devices, or anything about Viu!";
    } elseif (preg_match('/\b(subscribe|subscription|mag.?subscribe|plan|premium)\b/i', $question)) {
        $intent = 'subscribe';
    ...
    } elseif (preg_match('/\b(price|magkano|cost|how much|pricing)\b/i', $question)) {
        $intent = 'pricing';
    ...
    } elseif (preg_match('/\b(download|offline|save)\b/i', $question)) {
        $intent = 'download';
    ...
    } elseif (preg_match('/\b(cancel|unsubscribe|stop)\b/i', $question)) {
        $intent = 'cancel';
    ...
    } elseif (preg_match('/\b(device|devices|how many|screens)\b/i', $question)) {
        $intent = 'devices';
    ...
    } elseif (preg_match('/\b(genre|categories|type|content)\b/i', $question)) {
        $intent = 'genres';
        $answer = "We have K-dramas, C-dramas, anime, variety shows, movies, and more! Browse by genre in the app to find your favorites. 🎬";
    } elseif (preg_match('/\b(password|forgot|reset|login)\b/i', $question)) {
        $intent = 'account';
    ...
    } elseif (preg_match('/\b(subtitle|subtitles|dub|language)\b/i', $question)) {
        $intent = 'subtitles';
    ...
    } elseif (preg_match('/\b(quality|hd|4k|resolution|buffer|blurry|pixel)\b/i', $question)) {
        $intent = 'quality';
        $answer = "Streaming quality depends on your internet speed and subscription plan. Premium members get access to HD quality! 📺";
    } elseif (preg_match('/\b(thank|thanks|salamat)\b/i', $question)) {
        $intent = 'thanks';
